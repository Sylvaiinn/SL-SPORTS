...
export default function Home() {
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#f1f5f9', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {/* Nav */}
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(10,10,15,0.92)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '0 1.25rem' }}>
        <div style={{ maxWidth: '860px', margin: '0 auto', height: '3.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Link href="/login" style={{ textDecoration: 'none', display: 'inline-block', padding: '0.5rem 1.125rem', borderRadius: '0.625rem', border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.05)', color: '#f1f5f9', fontSize: '0.875rem', fontWeight: 600 }}>
            Se connecter
          </Link>
        </div>
      </nav>
      ...
      <section style={{ padding: '5rem 1.25rem 3rem', textAlign: 'center' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: '2.75rem', fontWeight: 900, lineHeight: 1.1, marginBottom: '1.25rem', letterSpacing: '-0.02em' }}>
            Suivez toutes vos <span style={{ color: '#60a5fa' }}>performances</span> sportives
          </h1>
          <p style={{ fontSize: '1.0625rem', color: '#94a3b8', lineHeight: 1.7, marginBottom: '2.25rem' }}>
            Une app tout-en-un pour tracker vos séances, générer des plans de natation, analyser vos runs et débloquer des trophées.
          </p>
      ...
      {/* Sports */}
      <section style={{ padding: '1.5rem 1.25rem 2rem', maxWidth: '860px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem' }}>
          {([
            { icon: Dumbbell, iconColor: '#a78bfa', border: 'rgba(139,92,246,0.2)', bg: 'rgba(139,92,246,0.05)', title: 'Musculation', desc: 'Créez vos séances, suivez le volume soulevé et progressez exercice par exercice.', features: ['Templates A/B/C inclus', 'Suivi du poids & reps', 'Historique filtrable'] },
            { icon: Waves,    iconColor: '#60a5fa', border: 'rgba(59,130,246,0.2)',  bg: 'rgba(59,130,246,0.05)',  title: 'Natation',     desc: 'Générez automatiquement des plans adaptés à votre niveau avec blocs structurés.',     features: ['Génération selon niveau', 'Endurance, Vitesse, Technique', 'Saisie manuelle possible'] },
            { icon: Footprints,iconColor: '#34d399', border: 'rgba(16,185,129,0.2)', bg: 'rgba(16,185,129,0.05)', title: 'Course à pied',desc: 'Enregistrez vos sorties, suivez votre allure et battez vos records personnels.', features: ['Records 1km → Marathon', 'Allure, BPM, D+', 'Trail, Fractionné, Tempo'] },
          ] as const).map(({ icon: Icon, iconColor, border, bg, title, desc, features }) => (
            <div key={title} style={{ borderRadius: '1.125rem', border: `1px solid ${border}`, background: bg, padding: '1.5rem' }}>
              <div style={{ width: '2.75rem', height: '2.75rem', borderRadius: '0.75rem', background: `${iconColor}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem' }}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                {features.map(f => (
                  <li key={f} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8125rem', color: '#64748b' }}>
                    <Check size={13} color={iconColor} style={{ flexShrink: 0 }} /> {f}
                  </li>
                ))}
              </ul>
            </div>
      ...
      {/* Features */}
      <section style={{ padding: '0 1.25rem 2rem', maxWidth: '860px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.875rem' }}>
          {([
            { icon: Trophy,    color: '#fbbf24', bg: 'rgba(245,158,11,0.08)',    border: 'rgba(245,158,11,0.15)',    title: 'Trophées',     desc: 'Débloquez des récompenses en atteignant vos objectifs : streaks, distances, records.' },
            { icon: TrendingUp,color: '#34d399', bg: 'rgba(16,185,129,0.08)',   border: 'rgba(16,185,129,0.15)',   title: 'Statistiques', desc: 'Visualisez votre progression avec des graphiques hebdomadaires.' },
            { icon: Users,     color: '#a78bfa', bg: 'rgba(139,92,246,0.08)',   border: 'rgba(139,92,246,0.15)',   title: 'Communauté',   desc: 'Fil d\'actualité partagé et profils publics pour suivre vos amis.' },
          ] as const).map(({ icon: Icon, color, bg, border, title, desc }) => (
            <div key={title} style={{ borderRadius: '1rem', border: `1px solid ${border}`, background: bg, padding: '1.125rem' }}>
              <Icon size={18} color={color} style={{ marginBottom: '0.5rem' }} />
      ...
      <section style={{ padding: '2rem 1.25rem 4rem', textAlign: 'center' }}>
        <div style={{ maxWidth: '420px', margin: '0 auto', padding: '2.5rem', borderRadius: '1.5rem', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
          ...
          <p style={{ fontSize: '0.9375rem', color: '#94a3b8', marginBottom: '1.75rem', lineHeight: 1.6 }}>Gratuit, sans publicité. Créez votre compte en 30 secondes.</p>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '1.25rem', textAlign: 'center' }}>
        <p style={{ fontSize: '0.8125rem', color: '#475569' }}>© 2025 SPORTS.SL — Suivez vos performances, battez vos records.</p>
      </footer>
    </div>
  )
}
