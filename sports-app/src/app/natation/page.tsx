...
import {
  ...
  Loader2, ChevronDown, History, Timer
} from 'lucide-react'
...
function BlockCard({ block }: { block: SwimBlock }) {
  ...
  const equipLabels: Record<SwimEquipment, string> = { palmes: '🦈', 'pull-buoy': '🟡', planche: '🏄' }
  return (
  ...
  return (
    <div className="page-enter">
      <div className="page-header">
        <h1>Natation</h1>
        <p>Génération de séances personnalisées</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          {EQUIPMENT_OPTIONS.map(({ value, label, icon }) => {
            ...
            return (
              <button key={value} onClick={() => toggleEquipment(value)} style={{ flex: 1, padding: '0.625rem', borderRadius: '0.75rem', border: `1px solid ${active ? 'var(--accent-violet)' : 'var(--border)'}`, background: active ? 'var(--accent-violet-glow)' : 'var(--bg-secondary)', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s', textAlign: 'center' }}>
                <div style={{ fontSize: '1.1rem', marginBottom: '0.2rem' }}>{icon}</div>
                <div style={{ fontSize: '0.75rem', fontWeight: active ? 700 : 500, color: active ? '#c4b5fd' : 'var(--text-muted)' }}>{label}</div>
              </button>
            )
          })}
        </div>
